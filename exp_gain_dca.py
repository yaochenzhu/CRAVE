...
debater_1_reflect_prompt = (
    "You are evaluating a debate on movie recommendations.\n"
    "Assess the reasoning and recommended movies of Debater 1 based on the user query: {user_query} and the initial argument by Debater 0: {debater_0}.\n"
    "Here is the reasoning and recommended movies of Debater 1: {debater_1}.\n"
    "Here are the ground truth movies the user wants to watch: {gt_items}.\n"
    ...
    "First, provide your judgment: success/failure, followed by '####'.\n"
    "Next, analyze why Debater 1's reasoning/recommendations succeed or fail based on the user query and ground truth movies, followed by '####'.\n"
    "Finally, summarize general guidelines for Debater 1 when debating for similar user queries, based on your analysis."
)

critic_reflect_prompt = (
    "You are evaluating a critique on a debate about movie recommendations.\n"
    "Assess the critique provided on the recommendations from two movie recommender systems based on the user query: {user_query}.\n"
    "Here is the reasoning and recommendations from Debater 0: {debater_0}\n"
    "Here is the reasoning and recommendations from Debater 1: {debater_1}\n"
    "Here is the critique you need to evaluate: {critic}\n"
    "Here are the ground truth movies the user wants to watch: {gt_items}.\n"
    "Determine if the critique's reasoning and recommendations are successful by checking consistency with the ground truth movies and overlap with highly scored movies.\n"
    "First, provide your judgment: success/failure, followed by '####'.\n"
    "Next, analyze why the critique's reasoning/scoring succeeds or fails based on the user query and ground truth movies, followed by '####'.\n"
    "Finally, summarize general guidelines for critiquing movie recommendations for similar user queries, based on your analysis."
)


def forward_pass_debater1(train_sim_idxes, train_data):
    EXSTING = {}
    threads, results = [], []

    for _, i in enumerate(tqdm(train_sim_idxes, 
                            total=len(train_sim_idxes), 
                            desc="forward pass for debater 1...")):   
        item = train_data[i]
        ...
        if len(threads) == n_threads:
            ...
            for res in results:
                index = res["index"]
                train_data[index][f"debater_1_fwd_from_llm"] = res
...
def forward_pass_critic(train_sim_idxes, train_data):
    EXSTING = {}
    threads, results = [], []

    for _, i in enumerate(tqdm(train_sim_idxes, 
                            total=len(train_sim_idxes), 
                            desc="forward pass for critic...")):   
        item = train_data[i]
        context = "\n".join([": ".join(rnd) for rnd in item['context_raw']])
        debater_0 = item["debater_0_fwd_from_llm"]['resp']['choices'][0]['message']['content']
        debater_1 = item["debater_1_fwd_from_llm"]['resp']['choices'][0]['message']['content']
        ...
        if len(threads) == n_threads:
            ...
            for res in results:
                index = res["index"]
                train_data[index][f"critic_fwd_from_llm"] = res
...
def backward_pass_debater1(train_sim_idxes, train_data):
    EXSTING = {}
    threads, results = [], []

    for _, i in enumerate(tqdm(train_sim_idxes, 
                            total=len(train_sim_idxes), 
                            desc="backward pass for debater 1...")):   
        item = train_data[i]
        context = "\n".join([": ".join(rnd) for rnd in item['context_raw']])
        debater_0 = item["debater_0_fwd_from_llm"]['resp']['choices'][0]['message']['content']
        debater_1 = item["debater_1_fwd_from_llm"]['resp']['choices'][0]['message']['content']
        ...
        prompt = debater_1_reflect_prompt
        ...
        if len(threads) == n_threads:
            ...
            for res in results:
                index = res["index"]
                train_data[index][f"debater_1_bwd_from_llm"] = res
...
def backward_pass_critic(train_sim_idxes, train_data):
    EXSTING = {}
    threads, results = [], []

    for _, i in enumerate(tqdm(train_sim_idxes, 
                            total=len(train_sim_idxes), 
                            desc="backward pass for the critic...")):   
        item = train_data[i]
        context = "\n".join([": ".join(rnd) for rnd in item['context_raw']])
        debater_0 = item["debater_0_fwd_from_llm"]['resp']['choices'][0]['message']['content']
        debater_1 = item["debater_1_fwd_from_llm"]['resp']['choices'][0]['message']['content']
        critic = item["critic_fwd_from_llm"]['resp']['choices'][0]['message']['content']
        gt_items = "\n".join(item['groundtruth'])
        ...
        if len(threads) == n_threads:
            ...
            for res in results:
                index = res["index"]
                train_data[index][f"critic_bwd_from_llm"] = res
...
if __name__ == '__main__':
    ...
    forward_pass_debater1(train_data, train_sim_idxes)
    forward_pass_critic(train_data, train_sim_idxes)
    
    backward_pass_debater1(train_data, train_sim_idxes)
    backward_pass_critic(train_data, train_sim_idxes)
